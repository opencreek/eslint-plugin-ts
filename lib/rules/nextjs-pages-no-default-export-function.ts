...
export default creator<Options, MessageIds>({
    ...
    meta: {
        ...
        fixable: "code",
        messages: {
            "no-default-export-function": "No 'default export function', because of react docgen imports.",
        },
        schema: [
            {},
        ],
        ...
        return {
            ExportDefaultDeclaration(node) {
                if (node.declaration.type === "FunctionDeclaration") {
                    const functionDeclaration = node.declaration as FunctionDeclaration
                    // check if we are a page

                    const fileName = context.getPhysicalFilename?.()
                    if (fileName?.includes("pages/")) {
                        const parent = node.parent as Program
                        if (parent == null) throw new Error("Can not resolve parent")
                        const assignmentToDefault = parent.body.some(it => {
                            if (it.type !== "ExpressionStatement" || (it as ExpressionStatement).expression.type !== "AssignmentExpression") {
                                return false
                            }
